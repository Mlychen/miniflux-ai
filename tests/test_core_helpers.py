...
class TestEntryRendering(unittest.TestCase):
    ...
    def test_build_summary_entry_webhook_fallback_fields(self):
        entry = {
        ...
        item = build_summary_entry(entry, 'summary')
        self.assertEqual(item['datetime'], '2026-02-25T01:02:03Z')
        self.assertEqual(item['category'], '')
        self.assertEqual(item['title'], 'title')
        self.assertEqual(item['content'], 'summary')
        self.assertEqual(item['url'], 'https://example.com/y')
