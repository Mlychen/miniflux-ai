...
def create_app(
    ...
    app_entries_repository = entries_repository or EntriesRepository(path='entries.json', lock=shared_lock)
    app_ai_news_repository = ai_news_repository or AiNewsRepository(path='ai_news.json', lock=shared_lock)
    app = Flask(__name__)
